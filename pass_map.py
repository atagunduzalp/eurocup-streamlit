...
def draw_player_pass_to_shot(passes, matches_dict):
    ...
    for i, thepass in passes.iterrows():
        ...
        if not pd.isna(thepass['pass_goal_assist']):
            passArrow = plt.Arrow(x, y, dx, dy, width=3, color="red")
            ax.add_patch(passArrow)

        elif not pd.isna(thepass['pass_assisted_shot_id']):
            passArrow = plt.Arrow(x, y, dx, dy, width=2, color="black")
            ax.add_patch(passArrow)

        elif thepass['pass_outcome'] != 'Incomplete':
            passArrow = plt.Arrow(x, y, dx, dy, width=1.2, color="wheat")
            ax.add_patch(passArrow)
        # else:
